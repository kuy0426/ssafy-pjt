<template>
  <div class="game-wrapper">
    ...
    <div class="typing-game-container">
      ...
      <div class="stats">
        <span>시간: {{ elapsedTime.toFixed(1) }}초</span>
    ...
    <div class="scoreboard">
      <h3>⏱️ TOP 3</h3>
      <ol>
        <li v-for="(r, i) in topResults" :key="i">
           [{{ r.user }}] {{ r.text }}<br />
          <small>{{ r.time.toFixed(1) }}초</small>
        </li>
        <li v-if="topResults.length === 0">아직 기록이 없습니다.</li>
      </ol>
    </div>
  </div>
</template>
...
.typing-area {
  ...
  font-size: 1rem; border: 2px solid #ccc; border-radius: 4px; resize: none;
}
.stats { display: flex; gap: 1rem; font-size: 1rem; }
.buttons { display: flex; gap: 1rem; }
button {
  padding: 0.6rem 1.2rem; background: #6c5ce7;
  color: #fff; border: none; border-radius: 4px; cursor: pointer;
}
button:disabled { opacity: 0.6; cursor: not-allowed; }
.scoreboard {
  width: 250px; background: #fff; padding: 1rem;
